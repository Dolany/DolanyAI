...
namespace AmandaSharp
{
    public class Event
    {
        ...
        [DllExport(ExportName = "Information", CallingConvention = CallingConvention.StdCall)]
        public static string Information(string AuthCode)
        {
            API.AuthCode = AuthCode;
            Plugin p = new Plugin();
            p.ID = "top.ox.AmandaSharp";//插件的唯一ID，内部标识
            p.Name = "测试SDK"; //插件的名称
            p.Author = "牛宝宝"; //这是插件的作者
            p.Version = "1.0.0"; // 插件的版本号
            p.Brief = "这是一个用来测试SDK的插件";  // 插件的简介&说明
            p.SDK = "3";// 插件的SDK版本号，唯一值，固定不变
            p.WindowsTitle = "{_TestMenu1=设置-真}";
        ...
        //初始化插件，插件加载时会调用此事件
        [DllExport(ExportName = "Event_Initialization", CallingConvention = CallingConvention.StdCall)]
        ...
        //插件被启用事件
        [DllExport(ExportName = "Event_pluginStart", CallingConvention = CallingConvention.StdCall)]
        public static int Event_pluginStart()
        {
            ...
            */
            API.SendLog("Path", Path, 0x00ff00);
        ...
        //插件被关闭事件
        [DllExport(ExportName = "Event_pluginStop", CallingConvention = CallingConvention.StdCall)]
        ...
        [DllExport(ExportName = "Event_GetNewMsg", CallingConvention = CallingConvention.StdCall)]
        public static int Event_GetNewMsg(int type, string GroupID, string FromQQ, string Msg, string MsgID)
        {
            ...
            */
            ...
            try
            {
                API.SendMsg(type, GroupID, FromQQ, infos.mems.Where(m => m.role == 0).First().nick);
            }
            catch (Exception ex)
            {
        ...
        //QQ财付通转账事件
        [DllExport(ExportName = "Event_GetQQWalletData", CallingConvention = CallingConvention.StdCall)]
        ...
        //管理员变动事件
        [DllExport(ExportName = "Event_AdminChange", CallingConvention = CallingConvention.StdCall)]
        ...
        //群成员增加事件
        [DllExport(ExportName = "Event_GroupMemberIncrease", CallingConvention = CallingConvention.StdCall)]
        ...
        //群成员减少事件
        [DllExport(ExportName = "Event_GroupMemberDecrease", CallingConvention = CallingConvention.StdCall)]
        ...
        //群添加事件
        [DllExport(ExportName = "Event_AddGroup", CallingConvention = CallingConvention.StdCall)]
        ...
        //被添加好友事件
        [DllExport(ExportName = "Event_AddFrinend", CallingConvention = CallingConvention.StdCall)]
        ...
        //成为了好友事件
        [DllExport(ExportName = "Event_BecomeFriends", CallingConvention = CallingConvention.StdCall)]
        ...
        //Cookies更新时会触发此事件
        [DllExport(ExportName = "Event_UpdataCookies", CallingConvention = CallingConvention.StdCall)]
        public static int Event_UpdataCookies()
        {
            return API.MSG_CONTINUE;// 返回0 下个插件继续处理该事件，返回1 拦截此事件不让其他插件执行
        }

        [DllExport(ExportName = "_TestMenu1", CallingConvention = CallingConvention.StdCall)]
        public static int _TestMenu1()
        {
            //MessageBox(0,TEXT("嘿，程序猿！"),TEXT("Msg:"),64);
            //new Setting().ShowDialog();

            SettingForm form = new SettingForm(profiles);
